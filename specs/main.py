...
class Show(wx.Frame):
    ...
    def brain(self, event):
        with open('E:\pycharm-project\头脑风暴\common\insert.py', 'r', encoding='UTF-8') as f:
            exec(f.read())

    def check1(self, event):
        with open('E:\pycharm-project\头脑风暴\common\check_sql_1.py', 'r', encoding='UTF-8') as f:
            exec(f.read())

    def storm(self, event):
        with open('E:\pycharm-project\头脑风暴\common\combine.py', 'r', encoding='UTF-8') as f:
            exec(f.read())

    def check2(self, event):
        with open('E:\pycharm-project\头脑风暴\common\check_sql_2.py', 'r', encoding='UTF-8') as f:
            exec(f.read())
